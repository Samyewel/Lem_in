...
/*   Updated: 2023/01/18 16:15:21 by egaliber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "lem_in.h"

void	move_ants_alrdy_in_play(t_ants *ants)
{
	ants->room_location = ants->next_room->name;
	ants->next_room = ants->next_room->next;
	ft_printf("L%d-%s", ants->ant_number, ants->room_location);
	//ants = ants->next;
}

void	send_next(t_ants *ants, t_queue *queue)
{
	ants->room_location = queue->name;
	ants->next_room = queue->next;
	ft_printf("L%d-%s\n", ants->ant_number, ants->room_location);
	ants->has_moved = true;
}

void	send_first(t_ants *ants, t_queue *queue)
{
	ants->room_location = queue->name;
	ants->next_room = queue->next;
	ft_printf("L%d-%s\n", ants->ant_number, ants->room_location);
...
void	printer(t_ants *ants, t_queue *queue, t_heads *heads, /*t_paths *paths,*/ t_data *data)
{
	...
	while (data->finished != data->ant_count)
	{
		...
		if (heads->ants_head->has_moved == true)
		{
			...
			while (ants->has_moved == true && data->finished != data->ant_count)
			{
				if (ants->has_moved == true && ants->has_finished == false)
					{
						move_ants_alrdy_in_play(ants);
						//printf("|%d|", ants->ant_number);
						if  (ft_atoi(ants->room_location) == data->room_count)//== 0 queue->end == true)
						{
		...
		if (ants->has_moved == false && ants->has_finished == false && ants->ant_number <= data->ant_count)
		{
			send_next(ants, queue);
				if (ants->ant_number == data->ant_count)
					ants = heads->ants_head;
				else
					ants = ants->next;
